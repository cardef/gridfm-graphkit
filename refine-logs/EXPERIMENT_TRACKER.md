...
| I003 | M1 | Implement deterministic partition | Kron + Quotient | source topology only | stable-ID ordering, fixed METIS seed, connected-cardinality repair, anchor tie-break, permutation test | MUST | RUNNING | initial `metis-contiguous-repair-v1` gate passed at `14abdf2` (job 55095), but all-topology job 55100 exposed singleton transport gaps; `metis-contiguous-covered-repair-v2` restores exact ceiling cardinality, enforces at least two buses per cell, and is undergoing clean verification |
...
| I010 | M3 | Implement compute and compatibility gates | all | synthetic + largest grids | cumulative-FLOP checkpoint tests, profiler cross-check, compile parity, upstream-flat load, clean clone, MLflow child-store create/search smoke | MUST | BLOCKED | refreshed CPU audit passed its available checks at `14abdf2` (job 55099); tracked upstream Flat checkpoint load passes; the A40 compile/profiler/all-core largest-grid gate waits for a clean audited geometry bundle at the final repair commit |
...
| R003 | M1 | Evaluate geometry candidates | Kron | source-development topology only | ≤12-policy table: residual, conditioning, nnz, FLOPs, build time, host peak | MUST | RUNNING | job 55104 authoritatively reran all 12 candidates at `d63b007` and retained policy hash `e17a2551d54cc78678f6ca55c056e149f01d52b9081e0b2793d3eea2bea35ec2`; the final covered-partition amendment requires one last clean rerun before downstream freeze |
...
| Current Abacus backend | every PF pool is complete; full audit 55081, target freeze 55087, split freeze 55088, and materialization 55089 passed; diagnostic geometry jobs 55090 and 55100 failed closed; I002–I006 jobs 55094–55098 and I010 CPU job 55099 passed for the initial repair | final covered-partition repair is in verification; both geometry failures remain in the denominator; no target efficacy was read and no treatment launch is authorized |

1. **Geometry repair:** pass clean I002–I006 gates, rerun all 12 R003 candidates, and rebuild every frozen topology under `metis-contiguous-covered-repair-v2`.
