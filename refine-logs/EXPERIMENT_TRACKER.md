...
| I003 | M1 | Implement deterministic partition | Kron + Quotient | source topology only | stable-ID ordering, fixed METIS seed, connected-cardinality repair, anchor tie-break, permutation test | MUST | PASS | `metis-contiguous-covered-repair-v3` passes 7/7 formal selected tests at clean `0c088bd` (job 55120), the all-55 topology-only audit (55118), and the all-topology geometry build (55126) |
...
| R005 | M4 | Freeze `C_cal` | Flat | source-development | treatment-blind throughput probe and 3-hour aggregate upper bound | MUST | PASS | authoritative A40 job 55497 freezes `C_cal=6.2e13` counted training FLOPs; guarded three-candidate upper bound is 10,792.84 seconds; strict gate record passes at `d8f7e3f` |
| C001 | M4 | Loss candidate 1 | Flat seed 0 | source-development | `C_cal` checkpoint, error, residual, GPU-hours | MUST | TODO | authorized by R005; fixed weights `[1,0.01]` |
| C002 | M4 | Loss candidate 2 | Flat seed 0 | source-development | `C_cal` checkpoint, error, residual, GPU-hours | MUST | TODO | authorized by R005; fixed weights `[1,0.1]` |
| C003 | M4 | Loss candidate 3 | Flat seed 0 | source-development | `C_cal` checkpoint, error, residual, GPU-hours | MUST | TODO | authorized by R005; fixed weights `[1,1]` |
...
| Current Abacus backend | every PF pool and freeze is complete; diagnostic geometry failures 55090/55100/55114 and launch-only R005 failure 55496 remain recorded; corrected A40 R005 job 55497 and strict gate 55499 pass | geometry bundle covers 55/55 with hash `ca7caa5f8a14db489b4977f1487af56f8e71530a564be36ef47b36426ffe71d0`; C001–C003 are next; no target efficacy was read and no treatment launch is authorized |

1. **C001–C003:** run the three fixed Flat seed-0 candidates to `C_cal`, evaluate only the two held-out source-development PF groups, and retain total GPU spend at or below three hours.
