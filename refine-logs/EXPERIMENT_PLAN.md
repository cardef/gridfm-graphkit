...
**Proposal source:** `refine-logs/FINAL_PROPOSAL.md`, SHA-256 `d58776c404f7d3db3a7499520caf68dd3eaafa918f89a943255b61e0229e767f`.
**Current status:** all 55 initial jobs generated PF-mode outputs; PF mode uses an internal OPF solve only to choose generator setpoints. Audit job 54676 failed closed on silent setpoint-solve attrition and a parallel-branch identity bug. The global deterministic retry-to-count repair completed both source-development pools, and job 54699 passed R003 from clean commit `8706bac25b3694803ee26d0f7a5ea8aefb6db230`: all 12 frozen candidates passed and policy `e17a2551d54cc78678f6ca55c056e149f01d52b9081e0b2793d3eea2bea35ec2` was selected. Recovery array 54691 completed 26 of 31 remaining pools; five stalled tasks were isolated in 54799, revealing deterministic small-deficit aggregate-profile truncation. The global full-horizon candidate repair passed 16 focused tests in job 54845, a direct compile/helper smoke in 54846, and Ruff 0.12 in 54851. Jobs 54702 and 54711–54714 remain fail-closed behind recovery and chain the full audit, target/split freeze, split tensors, and all-topology geometry. I010 CPU evidence passed its available checks at clean `d9f8229` in job 54722, and A40 job 54723 waits for geometry. I010, R005–R014, and all confirmatory runs remain BLOCKED until their typed PASS records exist.

**Partition amendment:** before any treatment run or target-efficacy read, all-topology job 55090 failed closed because the exact `ceil(rho N)` rule produced a one-cell hierarchy on two tiny frozen sources and seeded contiguous METIS returned missing/disconnected labels elsewhere. The amended global topology-only rule uses `max(2, ceil(rho N))` cells and versioned deterministic connected-cardinality repair. R003 and every affected I/R/system gate must be rerun at the final clean commit; no treatment is authorized by the diagnostic rebuild.

## Claim Map

| Claim | Why It Matters | Minimum Convincing Evidence | Linked Blocks |
|---|---|---|---|
| **C1 — compute-transfer scaling** | This is the paper's dominant contribution. It asks whether electrical hierarchy improves a domain-specific foundation model rather than a single-grid specialist. | At common cumulative-FLOP checkpoints, Kron must beat both Flat and Global at `G26/C` with an inverted one-sided 95% error bound below zero, retain a physical-residual upper bound no larger than `log(1.05)`, remain better at `C/2` and `C`, and show non-contracting point estimates from `G8` to `G26` and from the smallest to largest frozen size tercile. | B1, B3, B4 |
| **C2 — electrical operator rather than generic hierarchy** | Without this separation, a positive result supports only generic multiscale communication. | At `G26/C`, the inverted one-sided 95% bound for `log(error_Kron)-log(error_Quotient)` must be below zero with the same 5% residual gate. Partition, adapter, channel schema, coarse-node count, trainable capacity, data, and sparsity cap must match; realized nonzeros must be reported and charged in FLOPs rather than falsely equalized. | B2, B3, B4 |
...
| The gain comes from more data. | One fixed total scenario count across nested `G8 subset G16 subset G26`; only topology diversity changes. |
...
| Generated scenario pools | Exactly 2,331 successful PF outputs per source topology and 512 per source-development or target topology. Keep the frozen first-attempt config and base seed. If Datakit drops an internal OPF setpoint solve, append attempts under the single global `deterministic_retry_to_fixed_success_count_v1` rule: retry round `r>=1` uses `base_seed + 1000r`, requests exactly the remaining deficit, never overwrites prior successes, and records requested/before/after counts plus the retry-state hash. The 1,000 stride is globally fixed and keeps every frozen topology/round chunk-seed interval disjoint. Because Datakit otherwise truncates the aggregate profile to the small retry count, retries construct the original frozen full horizon and select the requested deficit without replacement using PCG64 at the retry seed (`full_horizon_pcg64_subsample_v1`); this rule is global and topology-blind. No topology-specific retry rule or solver setting is permitted. | data-inventory, config, generation-provenance, retry-state, and raw-data hashes |
| Scenario splits | Freeze from names, declared roles, and successful counts only, before any efficacy read. For each source case, PCG64 seed `20270714 + lexicographic topology ordinal` permutes IDs into 80% train, 10% validation, and the remainder test. Source-development and target cases are test-only. | split-policy, explicit ID tensors, and split-manifest hashes |
...
| Geometry policy | At most 12 joint `(rho, k_P, k_C, kappa)` policies; `m=max(2,ceil(rho N))` on the eligible inventory; one versioned topology-only connected-cardinality repair after seeded contiguous METIS; deterministic residual/FLOP choice from source topology only. | candidate table, repair-rule version, and selected-policy hash |
...
- **Dataset / split / task:** PF on nested `G8/G16/G26` sources; zero-shot evaluation on the frozen held-out target manifest.
...
- **Success criterion:** all five C1 conjunctive gates from the canonical proposal pass. The decisive comparison is `G26/C`; `C/2`, G8/G26, and size-tercile gates prevent checkpoint and scaling cherry-picking.
...
- **Dataset / split / task:** identical `G26` training data and frozen zero-shot targets used by B1.
...
- **Success criterion:** at `G26/C`, the inverted upper 95% error bound is below zero and the residual bound is no larger than `log(1.05)`.
...
| Core | G8 | G16 | G26 | Seeds | Runs | Role |
|---|---:|---:|---:|---|---:|---|
...
All 20 runs are mandatory once R014 passes. Operational launch order may prioritize G26 to expose resource failures, but target efficacy outputs remain sealed until the matrix completes. No arm may be dropped because an early result is unfavorable.
...
- one fixed scenario total across G8/G16/G26;
...
| Figure 2 — held-out error versus cumulative FLOPs for G8/G16/G26 | E001–E018, A001–A002 |
