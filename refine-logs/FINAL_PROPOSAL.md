...
2. run contiguous METIS on the unweighted bus graph with a fixed seed and `m = ceil(rho N)` cells;
3. choose one anchor per cell by maximum fine-graph degree, breaking ties by minimum bus ID; and
4. restore tensor order.
...
4. **Quotient-HGNS:** the `G32` mechanism control uses the same single adapter slot and coarse-block count with quotient geometry.
...
Construct nested source sets `G8 subset G16 subset G32` from distinct intact PGLib base cases. Line-outage variants do not count as distinct systems and are excluded from the core study. The total number of source training scenarios is identical across `G8`, `G16`, and `G32`; diversity therefore trades scenarios per topology rather than silently increasing data.
...
- Mandatory runs are `3 communication cores x 3 diversity levels x 2 seeds = 18`, plus two `G32` Quotient-HGNS seeds: 20 pretraining runs.
...
1. at `G32/C`, the inverted one-sided upper 95% bound for `d_e` is below zero against both Flat-HGNS and Global-HGNS;
...
5. the family-balanced `d_e` point estimate is negative at the diversity endpoints `G8` and `G32`, and the `G32` estimate is no closer to zero than the `G8` estimate; `G16` remains a mandatory reported point but is not an additional pass gate.

For the electrical-mechanism claim, the same exact group-level test must put the upper 95% bound for `log(error_Kron)-log(error_Quotient)` below zero at `G32/C`, while the same 5% residual gate holds. No effect threshold is selected after pilots.
...
- Spend at most 10 GPU-hours total before the campaign: at most 3 on Flat-HGNS loss-weight selection and the remainder on treatment-blind `G32` throughput profiles.
...
2. held-out error versus cumulative FLOPs for `G8/G16/G32`, with physical-residual gates;
